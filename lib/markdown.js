 ...
 * @type {Function}
 */
export const excludeDraftsFilter = (filename) => isDev || !filename.startsWith(DRAFT_PREFIX);
...
export const getMarkdownFilesInDirectory = (directory) => {
    return getFilesInDirectory(directory).filter(file => file.match(MD_EXT_REGEXP));
};
...
export const getDirStaticPaths = (directory, key = 'slug', fallback = false) => {
    ...
    const files = getMarkdownFilesInDirectory(directory).filter(excludeDraftsFilter);
...
export const getMarkdownData = async (filePath, fields = []) => {
    ...
    if (isDev && markdownData.slug && markdownData.slug.startsWith(DRAFT_PREFIX)) {
