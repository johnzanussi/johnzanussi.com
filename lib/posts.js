...
export const getPostData = async (slug) => {
    ...
    return {
        ...markdownData,
        url: getPostUrl(slug),
        absoluteUrl: getPostUrl(markdownData.slug, true),
    };
};
 ...
 *
 * @param  {String} slug           Slug for post file
 * @param  {Bool}   includeHost    Prepend the host to the URL
 * @return {String}                URL for post
 */
export const getPostUrl = (slug, includeHost = false) =>
    `${
        includeHost ? process.env.NEXT_PUBLIC_SITE_URL : ''
    }/${POSTS_PATH}/${slug}`;
...
export const getAllPostData = async () => {
    ...
    const postFiles =
        getMarkdownFilesInDirectory(POSTS_DIR).filter(excludeDraftsFilter);

    // Get data for only these fields
    const fields = ['slug', 'date', 'title', 'coverImage', 'excerpt'];
    ...
    return data.sort((p1, p2) =>
        compareDesc(parseISO(p1.date), parseISO(p2.date))
    );
};
...
export const getSiblingPosts = async (post) => {
    ...
    const postIndex = allPosts.findIndex(
        (allPost) => allPost.title === post.title
    );
 ...
 *
 * @return {Object} post A post object
 */
